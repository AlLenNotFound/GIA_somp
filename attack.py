...
def reconstruct_with_omp(args, device, sample, metric, model_wrapper: ModelWrapper):
    ...
    for k in range(Bsz):
        ...
        for i in range(len(candidate_pool)):
            ...
            num = _dot_sum(residual, candidate_grads[i], model_wrapper, ignore_final_layer=Fasle)
            den = (_dot_sum(candidate_grads[i], candidate_grads[i], model_wrapper, ignore_final_layer=Fasle)) ** 0.5
